...
class _RegistrationParams(object):
    id = [
        ...
        (_sorter_type_id_raw, None, backend.errors.ResourceCollisionError)
    ]
...
def test_model_register_sorter_type(model, id, attribute_type_ids):
    ...
    type_resources = model._map__resource_cls__resources[backend.resources.SorterType]
...
def test_model_retrieve_sorter_type(model, id):
    id_value, id_expected, id_error = id
    ...
    assert isinstance(resource, backend.resources.SorterType)
...
def test_model_release_sorter_type(model, id):
    id_value, id_expected, id_error = id
